 ...
 *
 * Three deliberate safeguards here, each defending against a real failure
 * mode — do not remove any of them when extending this route:
 *
 * 1. Signature verification on the raw body. Without this, anyone who
 *    knows your endpoint URL can POST a fake "charge.success" and unlock
 *    paid content for free.
 * 2. Server-to-server re-verification via verifyPaystackTransaction.
 *    The webhook body alone is not sufficient trust — verify independently
 *    against Paystack's API before mutating order state.
 * 3. Idempotency via paystackReference + a status check before mutating.
 *    Paystack retries webhooks; this handler WILL be called more than once
 *    for the same event. If you mark fulfillment/order status without
 *    checking current state first, a retried webhook can double-process.
 */
